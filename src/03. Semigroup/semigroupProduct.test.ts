...
describe("semigroupProduct", () => {
	it("semigroupProduct 인스턴스로 숫자 곱해서 반환", () => {
	...
	it("semigroupProduct 인스턴스로 숫자 곱하기 : (x * y) * z = x * (y * z) 만족", () => {
		...
		const y = 3;
		const z = 4;
		expect(semigroupProduct.concat(semigroupProduct.concat(x, y), z)).toBe(
			24
		);
		expect(semigroupProduct.concat(x, semigroupProduct.concat(y, z))).toBe(
			24
		);
	});
});
